...
## Коммивояжёр (Prolog) — запуск из Python

Добавлена вкладка **"ЛР5: Коммивояжёр (Prolog)"**, где матрица расстояний задаётся в GUI (Python), а решение ищется кодом на Prolog, вызываемым из Python через **Janus (`janus_swi`)**.

### Требования

- Установите **SWI‑Prolog 9.1.12+** (важно для Janus).
- Установите Python‑пакет:
  ```cmd
  pip install janus-swi
  ```
- Убедитесь, что `swipl` доступен из командной строки (в PATH). Если нет — переустановите SWI‑Prolog с добавлением в PATH или добавьте путь к `swipl.exe` вручную.

### Как пользоваться

1. Запустите GUI как обычно: `python flight_schedule_gui.py`
2. Откройте вкладку **"ЛР5: Коммивояжёр (Prolog)"**
3. Вставьте квадратную матрицу \(N \times N\) (разделители: пробел/запятая/`;`), \(N \le 10\)
4. Нажмите **"Решить"** — будет вызван Prolog‑предикат `answer/3`, а результат (путь/стоимость) отобразится справа.
...
- Проблемы с DLL (x64, WinError 193): [УСТРАНЕНИЕ_ПРОБЛЕМ.md](../УСТРАНЕНИЕ_ПРОБЛЕМ.md)
