...
public:
    // Конструкторы
    /**
     * \brief Создать пустой объект самолёта.
     *
     * Используется в случаях, когда номер и грузоподъёмность задаются позже
     * с помощью сеттеров.
     */
    Aircraft();

    /**
     * \brief Создать самолёт с заданным идентификатором и грузоподъёмностью.
     *
     * \param number Строковый идентификатор самолёта (например, "A-001");
     *               значение не обязано быть числом, допускаются буквы и прочие символы.
     *               Это же значение используется в C API и Python-обёртках как ID самолёта.
     * \param maxPayload Максимальная грузоподъёмность в килограммах; должно быть положительным числом.
     */
    Aircraft(const std::string& number, double maxPayload);

    /**
     * \brief Конструктор копирования: создать новый объект, копируя все поля из другого самолёта.
     */
